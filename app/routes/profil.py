...
def modifier_profil():
    ...
    # Mettre à jour les skills forts
    if 'skills_forts' in data:
        UserSkill.query.filter_by(user_id=current_user.id, type='fort').delete()
        for nom_skill in data['skills_forts']:
    ...
    if 'disponibilites' in data:
        ...
        for dispo in data['disponibilites']:
